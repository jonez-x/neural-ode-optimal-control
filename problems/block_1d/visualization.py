"""Visualisation for the 1D block-with-friction problem.

Four-panel figure:
    (1) State trajectories x(t), v(t)
    (2) Control trajectory u(t)
    (3) Phase portrait (x, v)
    (4) Training loss curves (log scale)
"""
...
def plot_results(
    ...
    """Create and return the four-panel result figure."""
    t_nn = trajectory.t
    z_nn = trajectory.z
    u_nn = trajectory.u
    ...
    _plot_states(axes[0, 0], t_nn, z_nn, reference, z_target, T)
    _plot_control(axes[0, 1], t_nn, u_nn, reference)
    _plot_phase(axes[1, 0], z_nn, reference, z0, z_target)
...
def _plot_states(ax, t_nn, z_nn, ref, z_target, T):
    ax.plot(t_nn, z_nn[:, 0], color=COLORS["nn_x"], label=r"$x(t)$ Neural ODE")
    ax.plot(t_nn, z_nn[:, 1], color=COLORS["nn_v"], label=r"$v(t)$ Neural ODE")
    if ref is not None:
        ax.plot(ref.t, ref.z[:, 0], color=COLORS["nn_x"], ls="--", alpha=0.7, label=r"$x(t)$ Reference")
        ax.plot(ref.t, ref.z[:, 1], color=COLORS["nn_v"], ls="--", alpha=0.7, label=r"$v(t)$ Reference")
...
def _plot_control(ax, t_nn, u_nn, ref):
...
def _plot_phase(ax, z_nn, ref, z0, z_target):
    ax.plot(z_nn[:, 0], z_nn[:, 1], color=COLORS["nn_x"], label="Neural ODE")
    if ref is not None:
        ax.plot(ref.z[:, 0], ref.z[:, 1], color=COLORS["nn_x"], ls="--", alpha=0.7, label="Reference")
